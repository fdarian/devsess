...
`defineDevCli` takes a `name`, a `dir`, a `platform`, optional `options`, and a `run` function:
...
- `platform` — the runtime devsess wires itself into: `{ services, runMain }` from either `@effect/platform-node` or `@effect/platform-bun`. devsess depends on neither package itself, so you supply whichever one you run scripts with — see [Getting Started](/getting-started#install).
...
1. It wires a layer — a `DevSessions` service rooted at `<dir>/.data/sessions`, merged with `platform.services`.
2. It wraps your entire `run` callback in `Effect.scoped(...)`.
3. It hands that to `platform.runMain`.
...
`makeDevSessionsLayer` only builds the `DevSessions` service itself — it still needs `FileSystem` and `Path` to do anything, which is what `NodeServices.layer` supplies here (`BunServices.layer` from `@effect/platform-bun` works the same way). Inside `defineDevCli` this wiring happens for you via `platform.services`; called directly, it's yours to provide.
