...
| `getExpectedMigrationCount` | `(migrationsFolder: string) => Effect<number, PgliteError \| PlatformError, FileSystem \| Path>` | Counts entries in `<migrationsFolder>/meta/_journal.json`; fails if the journal is missing |
| `readSqlMigrations` | `(dir: string) => Effect<string, PlatformError, FileSystem \| Path>` | Concatenates every `.sql` file in `dir`, sorted by filename, into one newline-joined string. `''` if `dir` doesn't exist or has no `.sql` files. Not called by anything else in this package — an exported standalone utility |
