...
| `getStickyPort()` | `() => Effect.Effect<number, PlatformError, FileSystem \| Path>` |
...
**`awaitRunning<T>(pkg)`** — reads *another* package's running-signal file, not this project's own. `pkg` starting with `.` or `/` resolves as a path relative to `dir`; anything else is resolved as a package name via `require.resolve('<pkg>/package.json')`. Waits using `fs.watch` on the parent directory (no polling), plus one immediate read to catch the already-published case. **No timeout** — waits indefinitely. If the sibling's `.data` directory doesn't exist yet — i.e. its dev script has never run — this crashes with an unrecoverable defect instead of waiting: `fs.watch` throws synchronously on a missing directory, and that throw isn't routed through the typed `PlatformError` channel.
...
namespace SessionState {
	function slot<T extends Schema.Top>(schema: T): {
		...
		write: (session: DevSession, data: T['Type']) => Effect.Effect<void, PlatformError, FileSystem | Path>
	}
}
```
