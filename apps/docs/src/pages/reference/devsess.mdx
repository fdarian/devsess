...
Reads the port already stored in `session`'s `sess.json`, passes it to [`get-port`](https://github.com/sindresorhus/get-port) as a preference, and writes back whichever port that call actually returns. **Not cached** — every call re-reads and re-asks; call it once per run and reuse the returned number. Built on `SessionState.slot` internally, so a `sess.json` corrupted by something outside `devsess` surfaces here as `SessionStateError` too.
