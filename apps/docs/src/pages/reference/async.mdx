...
Otherwise identical: `runManagedSubprocess` merges `env` over `process.env` and inherits stdio; `publishRunning` writes atomically, deletes its file on process exit, and reports readiness to the CLI daemon when launched by `devsess start`; `awaitRunning` has no timeout.
