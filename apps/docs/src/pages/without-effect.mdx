...
- **`createDevSessions(rootDir, services)` replaces `DevSessions` and `makeDevSessionsLayer`.** No service tag, no Layer of your own to build — for anything you do outside `defineDevCli`. Its `rootDir` is the sessions directory itself, not your project root: pass `<project>/.data/sessions`, or you'll get session directories somewhere `defineDevCli` never looks. `services` is the same platform layer you'd pass as `defineDevCli`'s `platform.services` — `NodeServices.layer` or `BunServices.layer`.
